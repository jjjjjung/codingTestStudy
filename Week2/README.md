...
-> Max = max(queue)
   ...
   if data == Max:
```
원하는 데이터의 출력 순서까지 print해주면 코드 완성

----
## 210718
- **2019 카카오 개발자 겨울 인턴쉽 - 크레인 인형뽑기 게임**

> - 행과 열,, 처음에 열심히 풀었다가 뭔가 이상해서 검색해보니 행과 열이 내가 생각한 것과 반대였음... 이럴수도 있나..
> - 그래도 순서만 바꿔주면 되어서 금방 해결함

입력 받는 값과 실제 배열의 인덱스 값은 차이가 있어서 먼저 *-1* 해준값을 정해줌
```py
row = moves[i] - 1
```
그리고 *if*문을 사용해서 입력받은 행에 인형이 있는지 확인해줌
```py
if board[len(board)-1][row] == 0:
    continue

if board[j][row] == 0:
    continue
```
값이 있으면 arr에 넣어주고 그 위치를 0으로 만들어줌

```py
arr.append(board[j][row]) # 값을 arr에 넣음
board[j][row] = 0
```
마지막으로 arr에 넣어주고 앞뒤로 같은 인형이 있는지 검사해줌
최소 2개가 있어야 되는 조건 잊지말고 챙겨주기
```py
if len(arr) > 1: # 길이가 최소 2가 되어야 검사
    if arr[-1] == arr[-2]: # 앞, 뒤의 값이 같으면 둘다 없애줌
        ...
        arr.pop(-1)  
```
`코드를 제대로 작성한 것 같은데 마지막에 통과가 안돼서 냅두고 알바 다녀와서 의자에 다시 앉았는데 앉자마자 for문 선언을 잘못 해준걸 발견함. 단순 실수,,, 꼼꼼하게 확인할것!`
